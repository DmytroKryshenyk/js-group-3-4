...
function addUser(event) {
  ...
  fetch('https://test-users-api.herokuapp.com/users', {
    ...
    .then(() => (resultBlock.innerHTML ='<p class="result">Result:</br>Користувача додано!</p>'))
    .catch(error => (result.textContent = 'Ошибка' + error));
}
...
function deleteUser(event) {
  ...
  fetch(`https://test-users-api.herokuapp.com/users${deleteUserInput.value}`, {
    ...
    .then(() => (resultBlock.innerHTML ='<p class="result">Result:</br>Користувача виделено!</p>'))
    .catch(error => (result.textContent = 'Ошибка' + error));
}
